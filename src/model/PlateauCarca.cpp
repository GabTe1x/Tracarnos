#include "../include/PlateauCarca.hpp"
#include <set>

PlateauCarca::PlateauCarca(){}

int PlateauCarca::peutPoser(TuileCarca & tuile, int x, int y)
{
...
std::ostream & operator <<( std::ostream& out, PlateauCarca& x ) {
    for(std::pair<std::pair<int, int>,TuileCarca*> it : x.tuiles){
        // Accessing KEY from element pointed by it.
        std::pair<int, int> paire = it.first;
        TuileCarca* dom= it.second;
        std::cout << "Carca. (x=: " << paire.first << "; y="<< paire.second << ")\n" << *dom << std::endl;
    }
