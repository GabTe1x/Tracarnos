...
int main() {
  std::cout << "Dominos (1), Trax (2) ou Carcasonne (3)?" << std::endl;
  int num;
  std::cin >> num;
  if (num == 1) {
    ...
    if (num == 1) {
      ...
      while (!ctrl.finDePartie()) {
        ...
        while (endTurn == false) {
          ...
          std::cout << "Joueur n°" << ctrl.getJoueurActuel()->getId()
                    << std::endl;
    ...
    if (num == 1) {
      ...
      while (!ctrl.finDePartie()) {
        ...
        while (endTurn == false) {
          ...
          std::cout << ": Nouvelle tuile:\n" << d << std::endl;
          int choix;
          std::cout << "placer (1), défausser (2), tourner(3) ou retourner(4)?"
                    << std::endl;
          std::cin >> choix;
          if (choix == 1) {
            int x;
    ...
    std::cin >> joueurs;
    int dominos;
    std::cout << "Nombre de tuile?" << std::endl;
    std::cin >> dominos;
    // int joueurs{2};int dominos{5};
    ControleurCarca ctrl;
    ctrl.commencer(joueurs, dominos);

    while (!ctrl.finDePartie()) {
      TuileCarca d = ctrl.piocher();
      bool endTurn{false};
      while (endTurn == false) {
        std::cout << ctrl << std::endl;
        std::cout << "Joueur n°" << ctrl.getJoueurActuel()->getId()
